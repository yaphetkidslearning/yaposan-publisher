...
import { createBillingPortal, createCheckoutSession, verifyStripeWebhook } from "./payments";
import { PLAN_CATALOG, resolveEntitlements, syncStripeSubscription } from "./billingPlatform";
...
let storage: ObjectStorage = config.storageDriver === "r2"
  ? new R2ObjectStorage({ endpoint: config.storageEndpoint ?? "https://storage.invalid", bucket: config.storageBucket, accessKeyId: config.r2AccessKeyId ?? "", secretAccessKey: config.r2SecretAccessKey ?? "", region: config.r2SigningRegion, publicBaseUrl: config.publicAssetBaseUrl })
  : new LocalObjectStorage(undefined, config.publicAssetBaseUrl);
export const setObjectStorage = (adapter: ObjectStorage) => { storage = adapter; };

const json=(res:ServerResponse,status:number,data:unknown,requestId:string)=>{res.writeHead(status,{"content-type":"application/json",...securityHeaders(requestId)});res.end(JSON.stringify(data))};
const text=(res:ServerResponse,status:number,data:string,requestId:string,contentType="text/plain; charset=utf-8")=>{res.writeHead(status,{"content-type":contentType,...securityHeaders(requestId)});res.end(data)};
const readBody=async(req:IncomingMessage)=>{const chunks:Buffer[]=[];let size=0;for await(const chunk of req){const value=Buffer.isBuffer(chunk)?chunk:Buffer.from(chunk);size+=value.length;if(size>config.maxRequestBodyBytes)throw new Error("PAYLOAD_TOO_LARGE");chunks.push(value)}return Buffer.concat(chunks)};
const bodyJson=async(req:IncomingMessage)=>{const raw=await readBody(req);const parsed=raw.length?JSON.parse(raw.toString()):{};const complexity=inspectJsonComplexity(parsed,{maxBodyBytes:config.maxRequestBodyBytes,maxJsonDepth:config.maxJsonDepth,maxJsonNodes:config.maxJsonNodes,maxStringLength:config.maxRequestBodyBytes,maxQueryParameters:config.maxQueryParameters,maxPathLength:2048});if(!complexity.valid)throw new Error(complexity.code);return parsed;};
const requestLimiter=new BoundedRateLimiter({limit:config.rateLimitPerMinute,windowMs:60_000,maxEntries:config.rateLimitMaxEntries});
...
export async function handleRequest(req:IncomingMessage,res:ServerResponse){await runtimeReady;const targetCheck=validateRequestTarget(req.url??"/",{maxBodyBytes:config.maxRequestBodyBytes,maxJsonDepth:config.maxJsonDepth,maxJsonNodes:config.maxJsonNodes,maxStringLength:config.maxRequestBodyBytes,maxQueryParameters:config.maxQueryParameters,maxPathLength:2048});const requestId=String(req.headers["x-request-id"]??randomUUID());const url=new URL(req.url??"/","http://localhost");const ip=clientIp(req,config.trustProxy);const finishMetric=runtimeMetrics.begin(req.method);res.once("finish",()=>finishMetric(res.statusCode));req.setTimeout(config.requestTimeoutMs,()=>{if(!res.headersSent)json(res,408,{error:{code:"REQUEST_TIMEOUT",message:"Request exceeded the configured time limit"}},requestId);req.destroy()});try{
 ...
 if(req.method==="POST"&&url.pathname==="/api/v1/assets/complete"){const b=await bodyJson(req);await requireWorkspaceAccess(db,userId,String(b.workspaceId),true);const asset=await db.insert("assets",{workspaceId:String(b.workspaceId),ownerUserId:userId,name:String(b.name),mimeType:String(b.contentType),size:Number(b.size),storageKey:String(b.key),checksum:b.checksum});return json(res,201,{asset},requestId)}
 if(req.method==="GET"&&url.pathname==="/api/v1/billing/plans")return json(res,200,{items:Object.values(PLAN_CATALOG)},requestId);
 ...
 if(req.method==="POST"&&url.pathname==="/api/v1/ai/generate"){const b=await bodyJson(req);const membership=(await db.find("memberships",m=>m.userId===userId))[0];if(!membership)return json(res,404,{error:{code:"NO_ORGANIZATION",message:"Organization not found"}},requestId);const org=await db.get("organizations",membership.organizationId);const sub=(await db.find("subscriptions",x=>x.organizationId===membership.organizationId))[0];const ent=resolveEntitlements(org!,sub);return json(res,200,await runAiGateway(db,{...b,userId,organizationId:membership.organizationId,plan:ent.plan}),requestId)};
 if(req.method==="POST"&&url.pathname==="/api/v1/billing/checkout"){const b=await bodyJson(req);const membership=(await db.find("memberships",m=>m.userId===userId))[0];if(!membership)return json(res,404,{error:{code:"NO_ORGANIZATION",message:"Organization not found"}},requestId);const sub=(await db.find("subscriptions",x=>x.organizationId===membership.organizationId))[0];if(!sub?.providerCustomerId)return json(res,409,{error:{code:"BILLING_CUSTOMER_REQUIRED",message:"Billing customer is not configured"}},requestId);const selection=validateCheckoutSelection({plan:String(b.plan??""),quantity:b.quantity,successUrl:String(b.successUrl??""),cancelUrl:String(b.cancelUrl??"")});return json(res,200,await createCheckoutSession({customerId:sub.providerCustomerId,priceId:selection.priceId,quantity:selection.quantity,successUrl:String(b.successUrl),cancelUrl:String(b.cancelUrl)}),requestId)}
 if(req.method==="POST"&&url.pathname==="/api/v1/billing/portal"){const b=await bodyJson(req);return json(res,200,await createBillingPortal(b.customerId,b.returnUrl),requestId)}
