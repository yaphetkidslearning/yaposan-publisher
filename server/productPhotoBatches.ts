...
export async function listProductPhotoBatches(db:DatabaseAdapter,userId:string,organizationId?:string){return (await db.find("jobs",j=>{const p=payloadOf(j);return !!p&&(p.userId===userId||(organizationId&&p.organizationId===organizationId));})).sort((a,b)=>b.createdAt.localeCompare(a.createdAt)).slice(0,50).map(publicBatch);}
...
export async function processProductPhotoBatch(db:DatabaseAdapter,storage:ObjectStorage,job:JobRecord){
  ...
  zip.file('YAPOSAN-BATCH-MANIFEST.json',JSON.stringify({phase:'91.8',jobId:job.id,strictWhite:p.strictWhite,items:p.items.map(x=>({name:x.safeName,status:x.status,whiteCompliance:x.whiteCompliance,error:x.error}))},null,2));
