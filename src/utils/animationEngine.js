export const DEFAULT_ANIMATION_PROJECT_SETTINGS = {
    duration: 5,
    fps: 30,
    loop: true,
    playbackSpeed: 1,
...
const ease = (t, easing) => {
    ...
    if (easing === "quint")
        return x ** 5;
    if (easing === "elastic")
        return x === 0 || x === 1 ? x : Math.pow(2, -10 * x) * Math.sin((x * 10 - .75) * (2 * Math.PI / 3)) + 1;
    if (easing === "bounce")
        return Math.abs(Math.sin(6.28 * (x + 1) * (1 - x)));
    return x < .5 ? 2 * x * x : 1 - Math.pow(-2 * x + 2, 2) / 2;
};
export function createElementAnimation(preset) {
...
export function evaluateAnimatedElement(element, time) {
    ...
    for (const animation of element.animations ?? []) {
        ...
        if (animation.direction === "reverse")
            local = 1 - local;
        if (animation.direction === "alternate" && Math.floor(cycle) % 2 === 1)
            local = 1 - local;
        if (animation.autoReverse)
            local = local < .5 ? local * 2 : (1 - local) * 2;
        const t = ease(local, animation.easing);
        if (raw < 0)
            local = 0;
        switch (animation.preset) {
            case "fade-in":
                next.opacity = element.opacity * t;
                break;
            case "fade-out":
                next.opacity = element.opacity * (1 - t);
                break;
            case "appear":
                next.opacity = raw >= 0 ? element.opacity : 0;
                break;
            case "disappear":
                next.opacity = raw >= 1 ? 0 : element.opacity;
                break;
            case "fly-in":
                next.x = element.x - (1 - t) * Math.max(100, element.width);
                next.opacity = element.opacity * t;
                break;
            case "fly-out":
                next.x = element.x + t * Math.max(100, element.width);
                next.opacity = element.opacity * (1 - t);
                break;
            case "zoom-in":
                next.width = element.width * Math.max(.01, t);
                next.height = element.height * Math.max(.01, t);
                break;
            case "zoom-out":
                next.width = element.width * (1 - .8 * t);
                next.height = element.height * (1 - .8 * t);
                next.opacity = element.opacity * (1 - t);
                break;
            case "grow-shrink":
                next.width = element.width * (1 + .12 * Math.sin(t * Math.PI));
                next.height = element.height * (1 + .12 * Math.sin(t * Math.PI));
                break;
            case "spin":
                next.rotation = element.rotation + 360 * t;
                break;
            case "float":
                next.y = element.y - Math.sin(t * Math.PI) * 24;
                break;
            case "bounce":
                next.y = element.y - Math.abs(Math.sin(t * Math.PI * 2)) * 32 * (1 - t * .4);
                break;
            case "pulse":
                next.opacity = element.opacity * (.65 + .35 * Math.sin(t * Math.PI));
                break;
            case "wipe":
                next.width = Math.max(1, element.width * t);
                break;
        }
        const localTime = clamp01(raw) * animation.duration;
        const keyed = interpolateKeyframes(animation, localTime);
        if (keyed) {
            if (keyed.x !== undefined)
                next.x = element.x + keyed.x;
            if (keyed.y !== undefined)
                next.y = element.y + keyed.y;
            if (keyed.scaleX !== undefined)
                next.width = element.width * keyed.scaleX;
            if (keyed.scaleY !== undefined)
                next.height = element.height * keyed.scaleY;
            if (keyed.rotation !== undefined)
                next.rotation = element.rotation + keyed.rotation;
            if (keyed.opacity !== undefined)
                next.opacity = element.opacity * clamp01(keyed.opacity);
        }
        if (animation.motionPath) {
            ...
            if (animation.motionPath.orientToPath)
                next.rotation = element.rotation + motion.angle;
        }
    }
    return next;
}
export function evaluateAnimatedPage(page, time) {
    return { ...page, elements: page.elements.map((element) => evaluateAnimatedElement(element, time)) };
}
export function getAnimationSettings(project) {
    return { ...DEFAULT_ANIMATION_PROJECT_SETTINGS, ...(project.animationSettings ?? {}) };
}
export function createAnimationKeyframe(time, value) {
    return { id: `key-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, time: Math.max(0, time), value, easing: "ease-in-out" };
}
export function addKeyframeToAnimation(element, animationId, keyframe) {
...
export function updateAnimationKeyframe(element, animationId, keyframeId, updates) {
...
export function removeAnimationKeyframe(element, animationId, keyframeId) {
    const animation = (element.animations ?? []).find((item) => item.id === animationId);
    if (!animation)
        return element;
    return updateElementAnimation(element, animationId, { keyframes: (animation.keyframes ?? []).filter((item) => item.id !== keyframeId) });
}
export function createMotionPath(points, name = "Custom Motion Path") {
...
export function setAnimationMotionPath(element, animationId, motionPath) {
    return updateElementAnimation(element, animationId, { motionPath });
}
export function duplicateElementAnimation(element, animationId) {
...
export function reverseElementAnimation(element, animationId) {
    const source = (element.animations ?? []).find((item) => item.id === animationId);
    if (!source)
        return element;
...
export function copyElementAnimations(element) {
    return { version: "19.1", animations: JSON.parse(JSON.stringify(element.animations ?? [])) };
}
export function pasteElementAnimations(element, clipboard, replace = false) {
...
export function staggerAnimations(elements, step = .15) {
...
const cubicBezierProgress = (t, bezier) => {
    if (!bezier)
        return t;
    const [, y1, , y2] = bezier;
    const u = 1 - t;
...
function interpolateKeyframes(animation, localTime) {
    const frames = [...(animation.keyframes ?? [])].sort((a, b) => a.time - b.time);
    if (!frames.length)
        return undefined;
    if (localTime <= frames[0].time)
        return frames[0].value;
    if (localTime >= frames[frames.length - 1].time)
    ...
    const result = {};
    const properties = ["x", "y", "scaleX", "scaleY", "rotation", "opacity"];
    for (const property of properties) {
        const start = left.value[property];
        const end = right.value[property];
        if (start !== undefined || end !== undefined)
            result[property] = (start ?? end ?? 0) + ((end ?? start ?? 0) - (start ?? end ?? 0)) * progress;
    }
    return result;
}
function pointOnMotionPath(path, progress) {
    ...
    for (let index = 0; index < lengths.length; index += 1) {
        if (distance <= lengths[index] || index === lengths.length - 1) {
            ...
            const ratio = clamp01(distance / Math.max(.001, lengths[index]));
