export function conditionalFormatForValue(value, rules) {
    const text = String(value ?? "");
    const number = Number(text);
    for (const rule of rules) {
        const target = Number(rule.value);
        const second = Number(rule.secondValue);
...
export function evaluateIfFormula(formula) {
...
const escapeXml = (value) => String(value ?? "")
    ...
    .replace(/"/g, "&quot;").replace(/'/g, "&apos;");
export function createProfessionalTableElement(id, zIndex, rows = 6, columns = 4, kind = "professional") {
    const cells = Array.from({ length: rows }, (_, row) => Array.from({ length: columns }, (_, column) => row === 0 ? `Column ${column + 1}` : ""));
    return {
        id, name: "Professional Table", type: "table",
...
export function formatAdvancedValue(value, format, locale = "en-US", currency = "USD") {
    ...
    if (format === "percent" && Number.isFinite(number))
        return new Intl.NumberFormat(locale, { style: "percent", maximumFractionDigits: 2 }).format(number > 1 ? number / 100 : number);
    if (format === "scientific" && Number.isFinite(number))
        return number.toExponential(3);
    if (format === "fraction" && Number.isFinite(number)) {
        const whole = Math.trunc(number);
        const fraction = Math.abs(number - whole);
        if (!fraction)
            return String(whole);
...
export function buildChartSvg(type, data, title = "Chart", width = 720, height = 420, options = {}) {
    ...
    const max = Number.isFinite(options.axisMax) ? Math.max(1, Math.abs(options.axisMax)) : rawMax;
    const min = Number.isFinite(options.axisMin) ? options.axisMin : 0;
    ...
    if (type === "pie" || type === "doughnut") {
        ...
        let angle = -Math.PI / 2;
        const cx = width * .38, cy = height * .54, radius = Math.min(plotW, plotH) * .34;
        ...
        if (type === "doughnut")
            body += `<circle cx="${cx}" cy="${cy}" r="${radius * .52}" fill="#fff"/>`;
        ...
        safe.forEach((item, i) => { const before = running; running += item.value; const y1 = margin.top + plotH - Math.max(0, before) / Math.max(max, Math.abs(running), 1) * plotH, y2 = margin.top + plotH - Math.max(0, running) / Math.max(max, Math.abs(running), 1) * plotH; const y = Math.min(y1, y2), h = Math.max(3, Math.abs(y2 - y1)); body += `<rect x="${margin.left + i * step + step * .18}" y="${y}" width="${step * .64}" height="${h}" fill="${item.value >= 0 ? '#0F766E' : '#DC2626'}"/><text x="${margin.left + i * step + step / 2}" y="${margin.top + plotH + 22}" text-anchor="middle" font-family="Arial" font-size="10">${escapeXml(item.label)}</text>`; });
    }
    else if (type === "box-plot") {
        const values = safe.map(x => x.value).sort((a, b) => a - b), q = (p) => values[Math.min(values.length - 1, Math.floor((values.length - 1) * p))] ?? 0;
        const min = q(0), q1 = q(.25), med = q(.5), q3 = q(.75), mx = q(1), scale = (v) => margin.left + (v - min) / (mx - min || 1) * plotW, y = height / 2;
        body += `<line x1="${scale(min)}" y1="${y}" x2="${scale(mx)}" y2="${y}" stroke="#334155" stroke-width="3"/><rect x="${scale(q1)}" y="${y - 55}" width="${Math.max(2, scale(q3) - scale(q1))}" height="110" fill="#CCFBF1" stroke="#0F766E" stroke-width="3"/><line x1="${scale(med)}" y1="${y - 55}" x2="${scale(med)}" y2="${y + 55}" stroke="#0F766E" stroke-width="4"/>`;
    }
    else if (type === "radar") {
        ...
        if (options.showGridlines !== false) {
            for (let g = 0; g <= 5; g += 1) {
                const gy = margin.top + plotH - (g / 5) * plotH;
                body += `<line x1="${margin.left}" y1="${gy}" x2="${margin.left + plotW}" y2="${gy}" stroke="#E2E8F0"/>`;
            }
        }
        ...
        if (type === "line" || type === "area" || type === "scatter" || type === "bubble") {
            ...
            if (type === "area")
                body += `<polygon points="${margin.left + step * .5},${margin.top + plotH} ${points} ${margin.left + step * (safe.length - .5)},${margin.top + plotH}" fill="#0F766E33"/>`;
            if (type !== "scatter" && type !== "bubble")
                body += `<polyline points="${points}" fill="none" stroke="#0F766E" stroke-width="4" stroke-linejoin="round"/>`;
            ...
            safe.forEach((item, i) => {
                if (type === "bar") {
                    const barH = plotH / safe.length * .62, y = margin.top + i * plotH / safe.length + 8, w = Math.max(1, Math.max(0, item.value) / max * plotW);
                    body += `<rect x="${margin.left}" y="${y}" width="${w}" height="${barH}" rx="5" fill="${chartPalette(i)}"/><text x="${margin.left - 8}" y="${y + barH / 2 + 4}" text-anchor="end" font-family="Arial" font-size="11">${escapeXml(item.label)}</text>`;
                }
                else {
                    const barW = step * .62, x = margin.left + i * step + (step - barW) / 2, h = Math.max(1, Math.max(0, item.value - min) / Math.max(1, max - min) * plotH), y = margin.top + plotH - h;
                    body += `<rect x="${x}" y="${y}" width="${barW}" height="${h}" rx="5" fill="${chartPalette(i)}"/><text x="${x + barW / 2}" y="${margin.top + plotH + 24}" text-anchor="middle" font-family="Arial" font-size="11">${escapeXml(item.label)}</text>${options.showDataLabels ? `<text x="${x + barW / 2}" y="${Math.max(margin.top + 12, y - 6)}" text-anchor="middle" font-family="Arial" font-size="11" fill="#0F172A">${escapeXml(item.value)}</text>` : ""}`;
                }
            });
        }
    }
...
export function createChartElement(id, zIndex, type, data, title) {
    const svgMarkup = buildChartSvg(type, data, title);
    return { id, name: title || "Chart", type: "svg", x: 80, y: 100, width: 720, height: 420, rotation: 0, zIndex, opacity: 1, svgMarkup, svgOriginalMarkup: svgMarkup, svgViewBox: "0 0 720 420", assetKind: "custom", editableVector: true, chartType: type, chartData: data, chartTitle: title };
}
export function monthMatrix(year, month, weekStartsMonday = false) {
...
export function createCalendarElement(id, zIndex, year, month, events = [], weekStartsMonday = false) {
...
export function createScheduleElement(id, zIndex, view) {
    ...
    const element = createProfessionalTableElement(id, zIndex, rows, headers.length, "schedule");
