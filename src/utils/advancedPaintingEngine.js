export const DEFAULT_ADVANCED_PAINTING_SETTINGS = {
    brushTipShape: "round",
    brushTipRoundness: 1,
    brushTipAngle: 0,
    brushTipSpacing: .12,
...
export function normalizeAdvancedPaintingSettings(value) {
    ...
    return {
        ...
        colorDepth: ([8, 16, 32].includes(Number(s.colorDepth)) ? Number(s.colorDepth) : 16),
    };
}
export function evaluatePressureCurve(pressure, curve) {
...
export function buildBrushTipDescriptor(settings) {
...
export function estimatePaintTileMemory(width, height, settings) {
    const s = normalizeAdvancedPaintingSettings(settings);
...
export function applyAdvancedPainting(element, settings) {
    const s = normalizeAdvancedPaintingSettings(settings);
    const recording = s.recordStrokes ? {
...
export function buildPhase184Certification(element) {
    return {
        phase: "18.4",
