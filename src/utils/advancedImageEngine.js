...
async function loadWebImage(uri) {
    if (typeof document === "undefined")
        throw new Error("Pixel processing is currently available on web.");
...
export async function calculateImageHistogram(uri) {
    ...
    const result = { red: Array(256).fill(0), green: Array(256).fill(0), blue: Array(256).fill(0), luminance: Array(256).fill(0) };
    for (let i = 0; i < pixels.length; i += 4) {
        if (pixels[i + 3] === 0)
            continue;
        result.red[pixels[i]] += 1;
        result.green[pixels[i + 1]] += 1;
        result.blue[pixels[i + 2]] += 1;
        result.luminance[Math.round(.2126 * pixels[i] + .7152 * pixels[i + 1] + .0722 * pixels[i + 2])] += 1;
    }
    return result;
}
export async function processImagePixels(uri, adjustments, quality = .95) {
    const image = await loadWebImage(uri);
    const canvas = document.createElement("canvas");
    ...
    for (let i = 0; i < data.length; i += 4) {
        ...
        r = interpolateCurve(r, adjustments.redCurve);
        g = interpolateCurve(g, adjustments.greenCurve);
        b = interpolateCurve(b, adjustments.blueCurve);
        if (vignette > 0) {
...
export async function localColorBackgroundCutout(uri, tolerance = 42) {
    const image = await loadWebImage(uri);
    const canvas = document.createElement("canvas");
    canvas.width = image.naturalWidth;
    canvas.height = image.naturalHeight;
