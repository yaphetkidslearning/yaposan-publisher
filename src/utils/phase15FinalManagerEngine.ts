...
import { refreshProjectDataSource, removeDataSource, type DataSourceRegistry } from "./phase15FunctionalCompletionEngine";
...
export function auditPhase15ExportReadiness(project: PublisherProject): Phase15ExportCheck[] {
  ...
  if (!checks.length) checks.push({ id: "ready", severity: "info", message: "All Phase 15 objects are ready for export." });
  return checks;
}
