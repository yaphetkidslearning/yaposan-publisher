...
export const DEFAULT_SPREAD_SETTINGS = { facingPages: false, startOnRight: true, pageGap: 24, spineGap: 12, allowShuffle: true };
export function normalizeLayoutProject(project) {
...
export function createLayer(project, name = "New Layer") {
    const layers = project.layers?.length ? project.layers : DEFAULT_LAYERS;
    const next = { id: uid("layer"), name, visible: true, locked: false, printable: true, color: "#7C3AED", order: layers.length };
...
export function moveLayer(project, layerId, direction) {
...
export function assignElementsToLayer(project, pageId, elementIds, layerId) {
...
function clonePage(page, name) {
    return { ...JSON.parse(JSON.stringify(page)), id: uid("master-page"), name, elements: page.elements.map((element) => ({ ...element, id: uid(element.type) })) };
}
export function createMasterFromPages(project, pageIds, name = "Master Spread") {
    const pages = pageIds.map((id) => project.pages.find((page) => page.id === id)).filter(Boolean);
    if (!pages.length)
        return project;
    const master = {
...
export function applyMasterToPages(project, masterId, pageIds) {
...
export function detachMasterFromPages(project, pageIds) {
...
export function reorderPage(project, pageId, direction) {
...
export function layoutCompletionDiagnostics(project) {
    const layers = project.layers ?? DEFAULT_LAYERS;
    ...
    const hiddenObjects = project.pages.flatMap((page) => page.elements).filter((element) => {
        const layer = layers.find((item) => item.id === element.layerId);
        return element.hidden || layer?.visible === false;
    }).length;
