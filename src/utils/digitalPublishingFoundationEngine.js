...
function uniqueTargetId(targets, preferred) {
    if (!targets.some((target) => target.id === preferred))
        return preferred;
    let suffix = 2;
    while (targets.some((target) => target.id === `${preferred}-${suffix}`))
        suffix += 1;
    return `${preferred}-${suffix}`;
}
export function createDefaultDigitalPublishingState(project, now = Date.now()) {
...
export function normalizeDigitalPublishingFoundation(project) {
    const now = Date.now();
...
export function addDigitalPublicationTarget(project, input) {
...
export function validateDigitalPublishingFoundation(project) {
    const normalized = normalizeDigitalPublishingFoundation(project);
    const state = normalized.digitalPublishingFoundation;
    const issues = [];
    if (!state.seo.title.trim())
        issues.push({ id: "seo-title", severity: "error", message: "A digital publication title is required.", fix: "Add an SEO title." });
    if (!state.seo.description.trim())
        issues.push({ id: "seo-description", severity: "warning", message: "SEO description is empty.", fix: "Add a concise publication description." });
    if (!state.accessibility.language.trim())
        issues.push({ id: "language", severity: "error", message: "Publication language is required.", fix: "Set a BCP 47 language code." });
    if (state.accessibility.minimumContrastRatio < 3)
        issues.push({ id: "contrast", severity: "error", message: "Minimum contrast ratio is below 3:1.", fix: "Use at least 3:1; 4.5:1 is recommended." });
    if (!state.targets.some((target) => target.enabled))
        issues.push({ id: "enabled-target", severity: "error", message: "At least one digital publication target must be enabled.", fix: "Enable or add a target." });
    if (!state.targets.some((target) => target.id === state.defaultTargetId))
        issues.push({ id: "default-target", severity: "error", message: "Default target does not exist.", fix: "Select an existing target as the default." });
    const seen = new Set();
    for (const target of state.targets) {
        if (seen.has(target.id))
            issues.push({ id: `duplicate-${target.id}`, severity: "error", targetId: target.id, message: `Duplicate target id: ${target.id}.`, fix: "Assign unique target identifiers." });
        seen.add(target.id);
        if (!target.outputPath.trim())
            issues.push({ id: `path-${target.id}`, severity: "error", targetId: target.id, message: `${target.name} has no output path.`, fix: "Set an output path." });
        if (target.width < 320 || target.height < 320)
            issues.push({ id: `viewport-${target.id}`, severity: "warning", targetId: target.id, message: `${target.name} uses a very small viewport.`, fix: "Use a viewport of at least 320 by 320." });
    }
    return issues;
}
export function createDigitalPublicationManifest(project) {
    const normalized = normalizeDigitalPublishingFoundation(project);
    const state = normalized.digitalPublishingFoundation;
...
export function exportDigitalPublishingFoundation(project) {
    const normalized = normalizeDigitalPublishingFoundation(project);
