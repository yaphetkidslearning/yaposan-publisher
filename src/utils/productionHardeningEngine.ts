...
export async function verifyLicenseEnvelope(envelope: LicenseEnvelope, publicKeyPem: string): Promise<boolean> {
  ...
  const key = await globalThis.crypto.subtle.importKey("spki", toArrayBuffer(decodeBase64(pem)), { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, false, ["verify"]);
  return globalThis.crypto.subtle.verify("RSASSA-PKCS1-v1_5", key, toArrayBuffer(decodeBase64(envelope.signature)), toArrayBuffer(new TextEncoder().encode(envelope.payload)));
}
