...
export type PaintBlendMode = "normal" | "multiply" | "screen" | "overlay" | "darken" | "lighten";
export type PaintBrushTip = "round" | "flat" | "calligraphy" | "charcoal" | "chalk" | "spray" | "watercolor" | "oil";

export type PaintingSettings = {
  presetId: string;
  tip: PaintBrushTip;
  color: string;
...
export type PaintPreset = Pick<PaintingSettings, "presetId" | "tip" | "size" | "opacity" | "flow" | "hardness" | "spacing" | "smoothing" | "scatter" | "angle" | "roundness" | "wetMix" | "texture" | "blendMode"> & { name: string; tool: "pencil" | "brush" | "calligraphy" | "marker" | "crayon" | "airbrush" | "highlighter" };
...
export const DEFAULT_PAINTING_SETTINGS: PaintingSettings = {
...
export function normalizePaintingSettings(value?:Partial<PaintingSettings>):PaintingSettings {
  return {
...
export function settingsFromPreset(presetId:string,current:PaintingSettings=DEFAULT_PAINTING_SETTINGS):PaintingSettings {
...
export function paintingElementPatch(settings:PaintingSettings):Partial<PublisherElement> {
...
export function smoothPaintPoints<T extends {x:number;y:number;pressure?:number}>(points:T[],amount:number):T[] {
  const strength=clamp(amount,0,1); if(points.length<3 || strength<=0) return points.map(p=>({...p}));
...
export function buildPaintManifest(element:PublisherElement) {
...
export type DynamicPaintPoint = {x:number;y:number;pressure?:number;velocity?:number;size?:number;opacity?:number;rotation?:number};
export function applyBrushDynamics<T extends {x:number;y:number;pressure?:number}>(points:T[],settings:PaintingSettings):DynamicPaintPoint[] {
  const s=normalizePaintingSettings(settings);
  return points.map((point,index)=>{
...
export function predictPaintEndpoint<T extends {x:number;y:number}>(points:T[],amount:number):T[] {
  const strength=clamp(amount,0,1);
  if(points.length<2 || strength===0) return points.map(point=>({...point}));
