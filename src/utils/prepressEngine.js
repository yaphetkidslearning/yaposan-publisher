export const DEFAULT_PREPRESS_SETTINGS = {
...
export function getPrepressSettings(project) {
    return { ...DEFAULT_PREPRESS_SETTINGS, ...(project.prepressSettings ?? {}) };
}
export function updatePrepressSettings(project, updates) {
...
function elementColors(element) {
    ...
    if (element.fillGradient)
        values.push(element.fillGradient.startColor, element.fillGradient.endColor);
    return values.filter((value) => Boolean(value && value !== "transparent"));
}
export function analyzeInkCoverage(project, settings = getPrepressSettings(project)) {
    const totals = [];
    for (const page of project.pages) {
        for (const element of page.elements) {
            ...
            for (const color of elementColors(element)) {
                ...
                if (cmyk)
                    totals.push(cmyk.c + cmyk.m + cmyk.y + cmyk.k);
            }
        }
    }
    return {
...
export function buildColorSeparations(project) {
    const process = [
        { name: "Cyan", key: "c" },
        { name: "Magenta", key: "m" },
        { name: "Yellow", key: "y" },
    ...
    const plates = process.map(({ name, key }) => ({ name, kind: "process", used: false, objectCount: 0, maximumCoverage: 0 }));
    for (const page of project.pages) {
        for (const element of page.elements) {
            if (element.hidden)
                continue;
...
export function buildProductionManifest(project, settings, reportData) {
    const fonts = new Set();
...
function issue(code, severity, message, pageId, elementId) {
    return { id: `${code}-${pageId ?? "project"}-${elementId ?? "all"}`, code, severity, message, pageId, elementId };
}
function touchesTrim(element, page, tolerance = 1) {
...
function extendsIntoBleed(element, page) {
...
export function buildBookletImposition(pageCount, signatureSize = 16, binding = "left") {
...
export function buildNUpImposition(pageCount, mode) {
...
export function buildImposition(project, settings = getPrepressSettings(project)) {
    if (settings.imposition === "booklet")
        return buildBookletImposition(project.pages.length, settings.signatureSize, settings.binding);
    if (settings.imposition === "two-up" || settings.imposition === "four-up" || settings.imposition === "step-repeat")
        return buildNUpImposition(project.pages.length, settings.imposition);
    return project.pages.map((_, index) => ({ sheet: index + 1, side: "front", slot: 0, pageNumber: index + 1, rotation: 0 }));
}
function isLikelyOutOfGamut(color, profile) {
...
export function analyzeSoftProof(project, settings = getPrepressSettings(project)) {
    let outOfGamutColors = 0;
    project.pages.forEach((page) => page.elements.forEach((element) => {
        if (element.hidden)
            return;
        elementColors(element).forEach((color) => { if (isLikelyOutOfGamut(color, settings.outputProfile))
            outOfGamutColors += 1; });
    }));
...
function normalizedName(element) {
    return element.name.trim().toLowerCase();
}
export function analyzeFinishing(project, settings = getPrepressSettings(project)) {
    ...
    project.pages.forEach((page) => page.elements.forEach((element) => {
        ...
        if (name.includes("dieline") || name.includes("die cut") || name.includes("cutcontour") || name.includes(settings.dielineSpotName.toLowerCase()))
            dielineObjects += 1;
        if (name.includes("fold") || name.includes("score"))
            foldObjects += 1;
        if (name.includes("perf"))
            perforationObjects += 1;
        if (name.includes("foil"))
            foilObjects += 1;
        if (name.includes("varnish") || name.includes("spot uv"))
            varnishObjects += 1;
    }));
    return { operations: settings.finishingOperations, dielineObjects, foldObjects, perforationObjects, foilObjects, varnishObjects };
}
export function buildImpositionSheets(project, settings = getPrepressSettings(project), slots = buildImposition(project, settings)) {
...
export function buildProductionFingerprint(project, settings = getPrepressSettings(project)) {
...
export function analyzePressReadiness(project, settings = getPrepressSettings(project)) {
    ...
    project.pages.forEach((page) => page.elements.forEach((element) => {
        ...
        if (element.type === "text") {
            ...
            if (size < settings.minimumTextSize)
                smallTextObjects += 1;
            ...
            if (reverse && size < settings.minimumReverseTextSize)
                reverseTextObjects += 1;
        }
        const lineWidth = element.borderWidth ?? 0;
        if ((element.type === "line" || lineWidth > 0) && lineWidth > 0 && lineWidth < settings.minimumLineWidth)
            hairlineObjects += 1;
    }));
...
export function runPrepress(project, settings = getPrepressSettings(project)) {
    const issues = [];
    if (settings.convertToCmyk && project.colorMode !== "CMYK")
        issues.push(issue("DOCUMENT_RGB", "warning", "Document is RGB and will be converted to CMYK during print output."));
    if (settings.pdfStandard === "PDF/X-1a" && !settings.flattenTransparency)
        issues.push(issue("PDFX1A_TRANSPARENCY", "error", "PDF/X-1a requires transparency flattening."));
    if (settings.inkLimit < 200 || settings.inkLimit > 400)
        issues.push(issue("INK_LIMIT", "warning", "Total ink limit should normally be between 200% and 400%."));
    if (settings.includeBleed && project.pages.some((page) => page.bleed <= 0))
        issues.push(issue("MISSING_DOCUMENT_BLEED", "error", "One or more pages have no bleed configured."));
    if (settings.imposition === "booklet" && project.pages.length % 4 !== 0)
        issues.push(issue("BOOKLET_BLANKS", "info", "Blank pages will be inserted so the booklet page count is divisible by four."));
    for (const page of project.pages) {
        if (page.width <= 0 || page.height <= 0)
            issues.push(issue("INVALID_PAGE_SIZE", "error", `Page “${page.name}” has an invalid size.`, page.id));
        for (const element of page.elements) {
            if (element.hidden)
                continue;
            if (element.type === "text") {
                const font = element.fontFamily?.trim();
                ...
                if (element.oversetText)
                    issues.push(issue("OVERSET_TEXT", "error", `Text frame “${element.name}” contains overset text.`, page.id, element.id));
            }
            if (element.type === "image") {
                if (!element.imageUri)
                    issues.push(issue("MISSING_IMAGE", "error", `Image “${element.name}” is missing its source.`, page.id, element.id));
                if (touchesTrim(element, page) && settings.includeBleed && !extendsIntoBleed(element, page))
                    issues.push(issue("IMAGE_NO_BLEED", "warning", `Image “${element.name}” touches trim but does not extend through bleed.`, page.id, element.id));
            }
            if ((element.opacity ?? 1) < 1 && settings.pdfStandard === "PDF/X-1a" && !settings.flattenTransparency)
                issues.push(issue("LIVE_TRANSPARENCY", "error", `Object “${element.name}” uses live transparency.`, page.id, element.id));
            if (element.hidden)
                issues.push(issue("HIDDEN_OBJECT", "info", `Object “${element.name}” is hidden and will not print.`, page.id, element.id));
    ...
    if (inkCoverage.objectsOverLimit > 0)
        issues.push(issue("INK_COVERAGE_EXCEEDED", "error", `${inkCoverage.objectsOverLimit} sampled color values exceed the ${settings.inkLimit}% total ink limit.`));
    if (settings.trapping && (settings.trapWidth <= 0 || settings.trapWidth > 2))
        issues.push(issue("INVALID_TRAP_WIDTH", "warning", "Trap width should normally be greater than 0 and no more than 2 points."));
    if (settings.registrationMarks && settings.markOffset < 6)
        issues.push(issue("MARK_OFFSET_TOO_SMALL", "warning", "Registration marks may overlap bleed because the mark offset is below 6 points."));
    if (settings.includeSlug && settings.slugSize < 12)
        issues.push(issue("SLUG_TOO_SMALL", "warning", "Slug area should normally be at least 12 points."));
    if (settings.generateSeparations && !settings.convertToCmyk)
        issues.push(issue("SEPARATIONS_RGB_OUTPUT", "warning", "Color separations are enabled while CMYK conversion is disabled."));
    const softProof = analyzeSoftProof(project, settings);
    if (settings.showGamutWarnings && softProof.outOfGamutColors > 0)
        issues.push(issue("OUT_OF_GAMUT_COLORS", "warning", `${softProof.outOfGamutColors} saturated colors may shift in the selected press profile.`));
    const finishing = analyzeFinishing(project, settings);
    if (settings.dielineRequired && finishing.dielineObjects === 0)
        issues.push(issue("MISSING_DIELINE", "error", `A dieline named “${settings.dielineSpotName}” is required but was not found.`));
    if (settings.finishingOperations.includes("fold") && finishing.foldObjects === 0)
        issues.push(issue("MISSING_FOLD_GUIDE", "warning", "Fold finishing is enabled but no fold or score guide was detected."));
    if (settings.finishingOperations.includes("perforate") && finishing.perforationObjects === 0)
        issues.push(issue("MISSING_PERFORATION", "warning", "Perforation finishing is enabled but no perforation object was detected."));
    if (settings.minimumLineWidth <= 0 || settings.minimumLineWidth > 2)
        issues.push(issue("LINE_WIDTH_STANDARD", "warning", "Minimum production line width should normally be greater than 0 and no more than 2 points."));
    const pressReadiness = analyzePressReadiness(project, settings);
    if (settings.screenFrequency < 65 || settings.screenFrequency > 300)
        issues.push(issue("SCREEN_FREQUENCY", "warning", "Screen frequency should normally be between 65 and 300 LPI."));
    if (settings.dotGain < 0 || settings.dotGain > 35)
        issues.push(issue("DOT_GAIN", "warning", "Dot gain should normally be between 0% and 35%."));
    if (pressReadiness.smallTextObjects > 0)
        issues.push(issue("SMALL_TEXT", "warning", `${pressReadiness.smallTextObjects} text objects are below the ${settings.minimumTextSize} pt minimum.`));
    if (pressReadiness.reverseTextObjects > 0)
        issues.push(issue("SMALL_REVERSE_TEXT", "error", `${pressReadiness.reverseTextObjects} reversed text objects are below the ${settings.minimumReverseTextSize} pt minimum.`));
    if (settings.enforceHairlineMinimum && pressReadiness.hairlineObjects > 0)
        issues.push(issue("HAIRLINE_TOO_THIN", "error", `${pressReadiness.hairlineObjects} strokes are below the ${settings.minimumLineWidth} pt production minimum.`));
    if (settings.approvalStatus === "press-approved" && !settings.approvedBy)
        issues.push(issue("APPROVER_MISSING", "warning", "Press approval is selected but no approver is recorded."));
...
export function applyPrintPreset(project, preset) {
    const presets = {
