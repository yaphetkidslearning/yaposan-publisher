...
import { queryMergeRecords, resolveBoundElement, type MailMergeProjectData, type MergeRecord } from "./mailMergeEngine";

export type MergeBatchMode = "individual" | "combined";
export type MergeBatchFormat = Extract<ExportFormat, "pdf" | "png" | "jpg" | "svg">;
export type MergeBatchSettings = {
  mode: MergeBatchMode;
  formats: MergeBatchFormat[];
  fileNameTemplate: string;
  includeDuplicates: boolean;
  generateReport: boolean;
};
export type MergeBatchHistoryItem = {
  id: string; createdAt: number; sourceId: string; sourceName: string; mode: MergeBatchMode;
  formats: MergeBatchFormat[]; recordCount: number; pageCount: number; files: string[];
  status: "completed" | "cancelled" | "failed"; durationMs: number; error?: string;
};
export type MergeBatchResult = { files: string[]; history: MergeBatchHistoryItem; projects: PublisherProject[] };

export const DEFAULT_MERGE_BATCH_SETTINGS: MergeBatchSettings = {
  mode: "individual", formats: ["pdf"], fileNameTemplate: "{project}-{record}-{format}-{number}",
  includeDuplicates: false, generateReport: true,
};

function safe(value: unknown) { return String(value ?? "").trim().replace(/[\\/:*?"<>|]+/g, "-").replace(/\s+/g, " ") || "record"; }
function recordLabel(record: MergeRecord, index: number) {
...
function resolvedPage(page: PublisherPage, source: NonNullable<ReturnType<typeof activeBatchSource>>, data: MailMergeProjectData, record: MergeRecord, index: number): PublisherPage {
...
function activeBatchSource(data: MailMergeProjectData) {
  const source = data.sources.find((item) => item.id === data.activeSourceId) ?? data.sources[0];
  if (!source) return undefined;
  return source;
}
export function getBatchRecords(data: MailMergeProjectData, includeDuplicates = false) {
  const source = activeBatchSource(data); if (!source) return [];
...
export function buildMergedProjects(project: PublisherProject, settings: MergeBatchSettings): PublisherProject[] {
  const data = project.mailMergeData; if (!data) return [];
  const source = activeBatchSource(data); if (!source) return [];
  ...
  if (settings.mode === "individual") return projects;
...
function exportOptions(project: PublisherProject, settings: MergeBatchSettings): ExportOptions {
...
export async function executeMergeBatch(project: PublisherProject, settings: MergeBatchSettings, onProgress: (progress: number, step: string) => void, isCancelled: () => boolean): Promise<MergeBatchResult> {
  const started = Date.now(), data = project.mailMergeData; if (!data) throw new Error("No mail merge data is configured.");
  const source = activeBatchSource(data); if (!source) throw new Error("Select a mail merge data source.");
  const projects = buildMergedProjects(project, settings); if (!projects.length) throw new Error("No records match the current search and filters.");
  let files: string[] = [];
  try {
    if (settings.mode === "combined") files = (await performExport(projects[0], exportOptions(projects[0], settings), onProgress, isCancelled)).files;
    else files = await performBatchExport({ projects, options: exportOptions(project, settings) }, onProgress, isCancelled);
