...
export function getLayoutSettings(page) {
    return { ...DEFAULT_LAYOUT_SETTINGS, ...(page.layoutSettings ?? {}), guides: page.layoutSettings?.guides ?? [] };
}
export function addLayoutGuide(page, orientation, position, name) {
    const settings = getLayoutSettings(page);
    const guide = { id: `guide-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, orientation, position: Math.max(0, position), name: name ?? `${orientation === "vertical" ? "V" : "H"} ${Math.round(position)}`, color: "#E11D48" };
    return { ...page, layoutSettings: { ...settings, guides: [...settings.guides, guide] } };
}
export function updateLayoutGuide(page, id, updates) {
    const settings = getLayoutSettings(page);
...
export function deleteLayoutGuide(page, id) {
    const settings = getLayoutSettings(page);
    return { ...page, layoutSettings: { ...settings, guides: settings.guides.filter((g) => g.id !== id) } };
}
export function snapCoordinate(value, targets, tolerance) {
    let best;
...
export function buildSnapTargets(page, axis) {
    const s = getLayoutSettings(page);
    const size = axis === "x" ? page.width : page.height;
    const targets = new Set([0, size / 2, size]);
    if (s.snapToMargins) {
        targets.add(page.margin);
        targets.add(size - page.margin);
    }
    if (s.snapToBleed) {
        targets.add(page.bleed);
        targets.add(size - page.bleed);
    }
    if (s.snapToSafeArea) {
        targets.add(s.safeArea);
        targets.add(size - s.safeArea);
    }
    if (s.snapToGuides)
        s.guides.filter((g) => !g.hidden && g.orientation === (axis === "x" ? "vertical" : "horizontal")).forEach((g) => targets.add(g.position));
    if (s.snapToGrid && s.gridSpacing > 0)
        for (let p = 0; p <= size; p += s.gridSpacing)
            targets.add(p);
    return [...targets];
}
