...
export const DEFAULT_ANIMATION_EXPORT_SETTINGS = {
...
const safe = (value) => value.replace(/[^a-z0-9-_]+/gi, "-").replace(/^-+|-+$/g, "").toLowerCase() || "yaposan-animation";
...
const safeJsonForScript = (value) => JSON.stringify(value)
    ...
    .replaceAll("\u2028", "\\u2028")
    .replaceAll("\u2029", "\\u2029");
const isSafeUrl = (value) => /^(https?:|mailto:|tel:|#|assets\/|data:image\/)/i.test(value);
const sanitizeSvgMarkup = (value) => value
    .replace(/<script[\s\S]*?<\/script>/gi, "")
    .replace(/\son[a-z]+\s*=\s*("[^"]*"|'[^']*'|[^\s>]+)/gi, "")
    .replace(/(?:href|xlink:href)\s*=\s*(["'])\s*javascript:[\s\S]*?\1/gi, 'href="#"');
const countAnimations = (project) => project.pages.reduce((total, page) => total + page.elements.reduce((sum, element) => sum + (element.animations?.length ?? 0), 0), 0);
const countInteractions = (project) => project.pages.reduce((total, page) => total + page.elements.reduce((sum, element) => sum + (element.interactions?.length ?? 0), 0), 0);
export function validateAnimationProject(project, settings) {
    const issues = [];
    const animationSettings = getAnimationSettings(project);
    const animationCount = countAnimations(project);
    const interactionCount = countInteractions(project);
    if (!project.pages.length)
        issues.push({ severity: "error", code: "NO_PAGES", message: "The project has no pages to export." });
    if (!animationCount)
        issues.push({ severity: "warning", code: "NO_ANIMATIONS", message: "No element animations were found." });
    if (settings.includeInteractions && !interactionCount)
        issues.push({ severity: "info", code: "NO_INTERACTIONS", message: "Interactive export is enabled but no object interactions were found." });
    if (settings.width < 320 || settings.height < 240)
        issues.push({ severity: "error", code: "INVALID_SIZE", message: "Export dimensions are too small." });
    if (settings.width * settings.height > 16_777_216)
        issues.push({ severity: "warning", code: "LARGE_CANVAS", message: "Very large canvas dimensions may exceed browser GPU or encoder limits." });
    if ((settings.format === "gif" || settings.format === "video") && animationSettings.duration * settings.fps > 1800)
        issues.push({ severity: "warning", code: "LARGE_MEDIA_EXPORT", message: "This media export contains more than 1,800 frames and may use significant memory." });
    project.pages.forEach((page) => page.elements.forEach((element) => {
        (element.animations ?? []).forEach((animation) => {
            if (animation.duration <= 0)
                issues.push({ severity: "error", code: "ZERO_DURATION", message: `${animation.name} has no duration.`, pageId: page.id, elementId: element.id });
            if ((animation.keyframes ?? []).some((frame) => frame.time > animation.duration))
                issues.push({ severity: "warning", code: "KEYFRAME_RANGE", message: `${animation.name} contains a keyframe beyond its duration.`, pageId: page.id, elementId: element.id });
        });
        (element.interactions ?? []).forEach((interaction) => {
            if (interaction.action === "open-url" && !interaction.url)
                issues.push({ severity: "warning", code: "MISSING_URL", message: `${interaction.name} has no URL.`, pageId: page.id, elementId: element.id });
            if (interaction.action === "open-url" && interaction.url && !isSafeUrl(interaction.url))
                issues.push({ severity: "error", code: "UNSAFE_URL", message: `${interaction.name} uses an unsupported or unsafe URL protocol.`, pageId: page.id, elementId: element.id });
            if (interaction.action === "go-to-page" && !interaction.targetPageId)
                issues.push({ severity: "warning", code: "MISSING_PAGE_TARGET", message: `${interaction.name} has no target page.`, pageId: page.id, elementId: element.id });
        });
    }));
...
export function estimateAnimationExport(project, settings) {
...
function vectorPath(element) {
    const points = (element.vectorNodes ?? element.vectorPoints ?? element.points ?? []);
    if (!points.length)
        return "";
    ...
    for (let i = 1; i < points.length; i += 1) {
        ...
        if (prev.outX !== undefined || point.inX !== undefined)
            d += ` C ${prev.outX ?? prev.x} ${prev.outY ?? prev.y} ${point.inX ?? point.x} ${point.inY ?? point.y} ${point.x} ${point.y}`;
        else
            d += ` L ${point.x} ${point.y}`;
    }
    if (element.vectorClosed)
        d += " Z";
    return d;
}
function tableMarkup(element) {
...
function elementInnerMarkup(element) {
    const source = element.rasterRenderedImageUri || element.rasterPreviewImageUri || element.imageUri || element.originalImageUri;
    if (element.type === "image" && source)
        return `<img src="${escapeHtml(source)}" alt="${escapeHtml(element.accessibilityLabel || element.name || "Image")}" style="width:100%;height:100%;object-fit:${element.imageFit === "stretch" ? "fill" : element.imageFit ?? "cover"};display:block"/>`;
    if (element.type === "svg" && element.svgMarkup)
        return `<div class="yp-svg" style="width:100%;height:100%">${sanitizeSvgMarkup(element.svgMarkup)}</div>`;
    if (element.type === "table")
        return tableMarkup(element);
    const path = vectorPath(element);
    if (path)
        return `<svg viewBox="0 0 ${Math.max(1, element.width)} ${Math.max(1, element.height)}" width="100%" height="100%" preserveAspectRatio="none"><path d="${escapeHtml(path)}" fill="${escapeHtml(element.fillColor ?? "none")}" fill-opacity="${element.vectorFillOpacity ?? 1}" stroke="${escapeHtml(element.borderColor ?? element.svgStroke ?? "#111827")}" stroke-opacity="${element.vectorStrokeOpacity ?? 1}" stroke-width="${element.borderWidth ?? element.svgStrokeWidth ?? 1}" stroke-linecap="${element.strokeCap ?? "butt"}" stroke-linejoin="${element.strokeJoin ?? "miter"}"/></svg>`;
    if (element.type === "circle")
        return `<div style="width:100%;height:100%;border-radius:50%;background:${cssGradient(element)}"></div>`;
    if (element.type === "line")
        return `<svg width="100%" height="100%"><line x1="0" y1="50%" x2="100%" y2="50%" stroke="${escapeHtml(element.borderColor ?? "#111827")}" stroke-width="${element.borderWidth ?? 2}"/></svg>`;
...
export function buildInteractiveHtml(project, settings) {
    const manifest = buildInteractiveManifest(project);
    const animationSettings = getAnimationSettings(project);
...
function svgElementMarkup(element) {
    ...
    const source = element.rasterRenderedImageUri || element.rasterPreviewImageUri || element.imageUri || element.originalImageUri;
    if (element.type === "image" && source)
        return `<g transform="${transform}" opacity="${element.opacity}"><image href="${escapeXml(source)}" width="${element.width}" height="${element.height}" preserveAspectRatio="${element.imageFit === "contain" ? "xMidYMid meet" : element.imageFit === "stretch" ? "none" : "xMidYMid slice"}"/></g>`;
    if (element.type === "svg" && element.svgMarkup)
        return `<g transform="${transform}" opacity="${element.opacity}">${sanitizeSvgMarkup(element.svgMarkup)}</g>`;
    const path = vectorPath(element);
    if (path)
        return `<g transform="${transform}" opacity="${element.opacity}"><path d="${escapeXml(path)}" fill="${escapeXml(element.fillColor ?? "none")}" stroke="${escapeXml(element.borderColor ?? element.svgStroke ?? "#111827")}" stroke-width="${element.borderWidth ?? element.svgStrokeWidth ?? 1}"/></g>`;
    if (element.type === "table") {
        const rows = element.tableCalculatedCells ?? element.tableCells ?? [];
        const rh = element.height / Math.max(1, rows.length);
        const cw = element.width / Math.max(1, Math.max(1, ...rows.map((row) => row.length)));
    ...
    const rx = element.type === "circle" ? element.width / 2 : element.borderRadius ?? 0;
    const ry = element.type === "circle" ? element.height / 2 : element.borderRadius ?? 0;
...
function frameSvg(project, page, time, width, height) {
    const animated = evaluateAnimatedPage(page, time);
    const sx = width / Math.max(1, page.width);
    const sy = height / Math.max(1, page.height);
...
function lzwEncode(indices, minCodeSize = 8) {
    ...
    const dict = new Map();
    const reset = () => { dict.clear(); for (let i = 0; i < clear; i += 1)
        dict.set(String(i), i); codeSize = minCodeSize + 1; next = end + 1; };
    const bytes = [];
    let bitBuffer = 0, bitCount = 0;
    const write = (code) => { bitBuffer |= code << bitCount; bitCount += codeSize; while (bitCount >= 8) {
    ...
    for (let i = 1; i < indices.length; i += 1) {
        const c = indices[i], key = `${prefix},${c}`;
        if (dict.has(key))
            prefix = key;
        else {
            write(dict.get(prefix) ?? 0);
            if (next < 4096) {
                dict.set(key, next++);
...
function encodeGif(frames, width, height, fps, loop) {
    ...
    if (loop)
        out.push(0x21, 0xFF, 0x0B, ...Array.from(new TextEncoder().encode("NETSCAPE2.0")), 0x03, 0x01, 0x00, 0x00, 0x00);
...
async function drawSvg(ctx, svg, width, height) { const blob = new Blob([svg], { type: "image/svg+xml" }); const url = URL.createObjectURL(blob); try {
    const image = new Image();
    await new Promise((resolve, reject) => { image.onload = () => resolve(); image.onerror = () => reject(new Error("Unable to render animation frame.")); image.src = url; });
...
async function renderGif(project, settings) { if (typeof document === "undefined")
    throw new Error("Real GIF export currently requires the web editor."); const canvas = document.createElement("canvas"); canvas.width = settings.width; canvas.height = settings.height; const ctx = canvas.getContext("2d", { willReadFrequently: true }); if (!ctx)
    throw new Error("Canvas rendering is unavailable."); const duration = getAnimationSettings(project).duration; const frames = []; for (const page of project.pages) {
    for (let f = 0; f < Math.max(1, Math.ceil(duration * settings.fps)); f += 1) {
        await drawSvg(ctx, frameSvg(project, page, f / settings.fps, settings.width, settings.height), settings.width, settings.height);
        frames.push(ctx.getImageData(0, 0, settings.width, settings.height).data);
    }
} return encodeGif(frames, settings.width, settings.height, settings.fps, settings.loop); }
async function renderVideo(project, settings) { if (typeof document === "undefined" || typeof MediaRecorder === "undefined")
    throw new Error("Real video export requires a browser with MediaRecorder support."); const canvas = document.createElement("canvas"); canvas.width = settings.width; canvas.height = settings.height; const ctx = canvas.getContext("2d"); if (!ctx)
    throw new Error("Canvas rendering is unavailable."); const stream = canvas.captureStream(settings.fps); const candidates = ["video/mp4;codecs=avc1.42E01E", "video/mp4", "video/webm;codecs=vp9", "video/webm"]; const mime = candidates.find(value => MediaRecorder.isTypeSupported(value)); if (!mime)
    throw new Error("This browser has no supported video encoder."); const chunks = []; const recorder = new MediaRecorder(stream, { mimeType: mime, videoBitsPerSecond: settings.quality === "ultra" ? 16000000 : settings.quality === "high" ? 10000000 : settings.quality === "standard" ? 6000000 : 3000000 }); recorder.ondataavailable = e => { if (e.data.size)
    chunks.push(e.data); }; const stopped = new Promise((resolve, reject) => { recorder.onstop = () => resolve(); recorder.onerror = () => reject(new Error("Video encoder failed.")); }); recorder.start(); const duration = getAnimationSettings(project).duration; for (const page of project.pages) {
    for (let f = 0; f < Math.max(1, Math.ceil(duration * settings.fps)); f += 1) {
        await drawSvg(ctx, frameSvg(project, page, f / settings.fps, settings.width, settings.height), settings.width, settings.height);
        await new Promise(r => setTimeout(r, 1000 / settings.fps));
    }
} recorder.stop(); await stopped; return { bytes: new Uint8Array(await new Blob(chunks, { type: mime }).arrayBuffer()), mimeType: mime, extension: mime.includes("mp4") ? "mp4" : "webm" }; }
function collectAssetUris(project) {
    const values = new Set();
    project.pages.forEach((page) => page.elements.forEach((element) => {
        [element.imageUri, element.originalImageUri, element.rasterOriginalImageUri, element.rasterPreviewImageUri, element.rasterRenderedImageUri].forEach((uri) => { if (uri)
            values.add(uri); });
    }));
    Object.values(project.embeddedFonts ?? {}).forEach((uri) => { if (uri)
        values.add(uri); });
    return [...values];
}
async function fetchAssetBytes(uri) {
    ...
    if (!response.ok)
        throw new Error(`Unable to package asset: ${uri}`);
    return new Uint8Array(await response.arrayBuffer());
}
function assetExtension(uri) { const clean = uri.split("?")[0]; const match = clean.match(/\.([a-z0-9]{2,5})$/i); return match?.[1]?.toLowerCase() || (uri.startsWith("data:image/svg") ? "svg" : uri.startsWith("data:image/png") ? "png" : uri.startsWith("data:image/jpeg") ? "jpg" : "bin"); }
async function presentationZip(project, settings) {
    const JSZip = (await import("jszip")).default;
    const zip = new JSZip();
    const packaged = structuredClone(project);
    const assetMap = new Map();
    let assetIndex = 0;
    for (const uri of collectAssetUris(project)) {
        try {
            const path = `assets/asset-${String(++assetIndex).padStart(4, "0")}.${assetExtension(uri)}`;
            zip.file(path, await fetchAssetBytes(uri));
            assetMap.set(uri, path);
        }
        catch { /* validation report retains external asset warning */ }
    }
    packaged.pages.forEach((page) => page.elements.forEach((element) => { const keys = ["imageUri", "originalImageUri", "rasterOriginalImageUri", "rasterPreviewImageUri", "rasterRenderedImageUri"]; keys.forEach((key) => { const uri = element[key]; if (uri && assetMap.has(uri))
        element[key] = assetMap.get(uri); }); }));
    if (packaged.embeddedFonts)
        Object.entries(packaged.embeddedFonts).forEach(([name, uri]) => { if (assetMap.has(uri))
            packaged.embeddedFonts[name] = assetMap.get(uri); });
    zip.file("index.html", buildInteractiveHtml(packaged, settings));
    zip.file("manifest.json", JSON.stringify({ ...buildInteractiveManifest(packaged), phase: "19.6", assets: [...assetMap.values()] }, null, 2));
    zip.file("project.json", JSON.stringify(packaged, null, 2));
    zip.file("README.txt", "Yaposan Phase 19.6 production-hardened interactive presentation package. Open index.html in a modern browser. Assets and embedded fonts are stored in the assets folder.");
    return zip.generateAsync({ type: "uint8array", compression: "DEFLATE", compressionOptions: { level: settings.optimizeAssets ? 9 : 3 } });
}
async function deliver(data, filename, mimeType) { if (typeof document !== "undefined") {
    const blob = new Blob([data], { type: mimeType }), url = URL.createObjectURL(blob), a = document.createElement("a");
    a.href = url;
    a.download = filename;
    a.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
    return filename;
} const [{ File: ExpoFile, Paths }, Sharing] = await Promise.all([import("expo-file-system"), import("expo-sharing")]); const file = new ExpoFile(Paths.cache, filename); file.create({ overwrite: true, intermediates: true }); file.write(typeof data === "string" ? data : data); if (await Sharing.isAvailableAsync())
    await Sharing.shareAsync(file.uri, { mimeType }); return file.uri; }
export function certifyAnimationProduction(project, settings) {
    const report = validateAnimationProject(project, settings);
    ...
    if (externalAssets.length)
        warnings.push("External assets must remain reachable during ZIP/media generation.");
    return { phase: "19.6", generatedAt: new Date().toISOString(), ready: checks.every((check) => check.passed) && report.valid, checks, warnings };
}
export async function performAnimationExport(project, settings) { const report = validateAnimationProject(project, settings); if (!report.valid)
    throw new Error(report.issues.filter(i => i.severity === "error").map(i => i.message).join(" ")); const base = safe(project.name); let filename = "", mimeType = "", data; if (settings.format === "interactive-html") {
    filename = `${base}-interactive.html`;
    mimeType = "text/html";
    data = buildInteractiveHtml(project, settings);
}
else if (settings.format === "presentation-zip") {
    filename = `${base}-presentation.zip`;
    mimeType = "application/zip";
    data = await presentationZip(project, settings);
}
else if (settings.format === "gif") {
    filename = `${base}-animation.gif`;
    mimeType = "image/gif";
    data = await renderGif(project, settings);
}
else if (settings.format === "video") {
    const video = await renderVideo(project, settings);
    filename = `${base}-animation.${video.extension}`;
    mimeType = video.mimeType;
    data = video.bytes;
}
else {
    filename = `${base}-presentation.json`;
    mimeType = "application/json";
    data = JSON.stringify({ version: "19.6", generatedAt: new Date().toISOString(), project, interactiveManifest: buildInteractiveManifest(project), animationSettings: getAnimationSettings(project), exportSettings: settings, validation: report, certification: certifyAnimationProduction(project, settings) }, null, 2);
} const deliveredTo = await deliver(data, filename, mimeType); return { filename, mimeType, report, deliveredTo, fallbackUsed: settings.format === "video" && !mimeType.includes("mp4"), certification: certifyAnimationProduction(project, settings) }; }
export function buildAnimationExportPackage(project, settings) { const report = validateAnimationProject(project, settings); const base = safe(project.name); if (settings.format === "interactive-html")
    return { filename: `${base}-interactive.html`, mimeType: "text/html", contents: buildInteractiveHtml(project, settings), report, format: settings.format }; const payload = { version: "19.6", generatedAt: new Date().toISOString(), project, interactiveManifest: buildInteractiveManifest(project), animationSettings: getAnimationSettings(project), exportSettings: settings, validation: report }; return { filename: `${base}-${settings.format}.json`, mimeType: "application/json", contents: JSON.stringify(payload, null, 2), report, format: settings.format }; }
