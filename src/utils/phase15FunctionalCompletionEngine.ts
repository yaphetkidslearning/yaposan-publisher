...
import { buildChartSvg, createCalendarElement, type ChartDatum, type ChartType } from "./dataVisualizationEngine";
import { refreshLinkedElement, type LinkedDataSource } from "./professionalDataObjectsEngine";

export type FormulaError = "#REF!" | "#VALUE!" | "#DIV/0!" | "#CIRC!" | "#NAME?";
export type FormulaResult = string | number | FormulaError;
export type DataSourceRegistry = Record<string, LinkedDataSource>;

const colToIndex = (letters: string) => letters.toUpperCase().split("").reduce((n, c) => n * 26 + c.charCodeAt(0) - 64, 0) - 1;
...
const numeric = (value: unknown): number => {
  ...
  if (!Number.isFinite(parsed)) throw new Error("VALUE");
  return percent ? parsed / 100 : parsed;
};

function getCell(cells: string[][], ref: string): string {
...
function expandRange(cells: string[][], range: string): string[] {
  ...
  const result: string[] = [];
  for (let row = minRow; row <= maxRow; row += 1) for (let col = minCol; col <= maxCol; col += 1) result.push(cells[row]?.[col] ?? "");
  return result;
}

export function evaluateTableFormula(formula: string, cells: string[][], stack = new Set<string>()): FormulaResult {
  ...
  if (fn) {
    ...
    if (fn[1].toUpperCase() === "COUNT") return values.length;
    if (!values.length) return 0;
    if (fn[1].toUpperCase() === "SUM") return values.reduce((a, b) => a + b, 0);
    if (fn[1].toUpperCase() === "AVERAGE") return values.reduce((a, b) => a + b, 0) / values.length;
    if (fn[1].toUpperCase() === "MIN") return Math.min(...values);
    return Math.max(...values);
  }
  try {
    const replaced = expression.replace(refPattern, (match) => {
      if (stack.has(match.toUpperCase())) throw new Error("CIRC");
      const raw = getCell(cells, match);
      if (raw === "#REF!") throw new Error("REF");
      if (raw.startsWith("=")) {
        const next = new Set(stack); next.add(match.toUpperCase());
    ...
    if (!/^[0-9+\-*/().\s]+$/.test(replaced)) return "#NAME?";
    const value = Function(`"use strict";return (${replaced})`)();
    if (!Number.isFinite(value)) return value === Infinity ? "#DIV/0!" : "#VALUE!";
    return value;
  } catch (error) {
    const message = String(error);
    if (message.includes("CIRC")) return "#CIRC!";
    if (message.includes("REF")) return "#REF!";
    return "#VALUE!";
  }
}

export function recalculateTable(element: PublisherElement): PublisherElement {
  if (element.type !== "table" || !element.tableCells) return element;
...
export function updateChartElement(element: PublisherElement, input: { type?: ChartType; title?: string; data?: ChartDatum[]; legendPosition?: "none"|"top"|"right"|"bottom"|"left"; axisMin?: number; axisMax?: number; showGridlines?: boolean; showDataLabels?: boolean }): PublisherElement {
  if (!element.chartType) return element;
...
export function regenerateCalendarElement(element: PublisherElement, overrides: Partial<Pick<PublisherElement, "calendarYear" | "calendarMonth" | "calendarEvents">> = {}): PublisherElement {
  if (element.calendarView !== "month") return { ...element, ...overrides };
  ...
  const rebuilt = createCalendarElement(element.id, element.zIndex, year, month, events as any);
...
export function upsertCalendarEvent(element: PublisherElement, event: NonNullable<PublisherElement["calendarEvents"]>[number]): PublisherElement {
...
export const removeCalendarEvent = (element: PublisherElement, eventId: string): PublisherElement => regenerateCalendarElement(element, { calendarEvents: (element.calendarEvents ?? []).filter((event) => event.id !== eventId) });
export function moveCalendarEvent(element: PublisherElement, eventId: string, date: string, time?: string): PublisherElement {
...
export function registerDataSource(registry: DataSourceRegistry, source: LinkedDataSource): DataSourceRegistry { return { ...registry, [source.id]: { ...source } }; }
export function removeDataSource(registry: DataSourceRegistry, sourceId: string): DataSourceRegistry { const next = { ...registry }; delete next[sourceId]; return next; }
export function dataSourceDependents(project: PublisherProject, sourceId: string): PublisherElement[] { return project.pages.flatMap((page) => page.elements.filter((element) => element.linkedDataSource?.id === sourceId)); }
export function refreshProjectDataSource(project: PublisherProject, source: LinkedDataSource): PublisherProject {
...
export function validateDataSourceRegistry(project: PublisherProject): Array<{ sourceId: string; status: "ready" | "missing"; dependentIds: string[] }> {
...
export function addDiagramNode(elements: PublisherElement[], groupId: string, nextId: () => string, title = "New Node"): PublisherElement[] {
  ...
  const nodeId = nextId();
  const node = { id: nodeId, name: title, type: "rectangle", x: group[0]?.x ?? 120, y: maxY + 30, width: 180, height: 72, rotation: 0, zIndex: maxZ + 2, opacity: 1, shapeType: "rounded-rectangle", fillColor: "#CCFBF1", borderColor: "#0F766E", borderWidth: 2, text: title, textColor: "#0F172A", fontSize: 18, groupId, diagramType: group[0]?.diagramType, diagramRole: "node" } as PublisherElement;
  const parent = group.filter((item) => item.diagramRole === "node").at(-1);
  const connector = parent ? { id: nextId(), name: `${parent.name} to ${title}`, type: "line", x: parent.x + parent.width / 2, y: parent.y + parent.height, width: Math.max(1, node.x + node.width / 2 - (parent.x + parent.width / 2)), height: Math.max(1, node.y - (parent.y + parent.height)), rotation: 0, zIndex: maxZ + 1, opacity: 1, borderColor: "#0F766E", borderWidth: 2, groupId, diagramType: group[0]?.diagramType, diagramRole: "connector", diagramFromId: parent.id, diagramToId: node.id } as PublisherElement : null;
  return [...elements, ...(connector ? [connector] : []), node];
}
export function deleteDiagramNode(elements: PublisherElement[], nodeId: string): PublisherElement[] { return elements.filter((element) => element.id !== nodeId && (element.diagramFromId !== nodeId && element.diagramToId !== nodeId)); }
export function relayoutDiagram(elements: PublisherElement[], groupId: string, direction: "vertical"|"horizontal" = "vertical"): PublisherElement[] {
