...
function hash(value, prefix = "web") { let h = 2166136261; for (let i = 0; i < value.length; i += 1) {
    h ^= value.charCodeAt(i);
    h = Math.imul(h, 16777619);
} return `${prefix}-${(h >>> 0).toString(16).padStart(8, "0")}`; }
function escapeHtml(value) { return value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\"/g, "&quot;").replace(/'/g, "&#39;"); }
function slug(value) { return value.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "page"; }
function routeFor(page, index) { return index === 0 ? "/" : `/${slug(page.name)}/`; }
function fileFor(page, index) { return index === 0 ? "index.html" : `${slug(page.name)}/index.html`; }
function cssValue(value, fallback = 0) { return Number.isFinite(value) ? value : fallback; }
function renderElement(element) {
    ...
    if (element.type === "text")
        return `<div ${attrs}><span style="font-family:${escapeHtml(element.fontFamily || "Arial")};font-size:${cssValue(element.fontSize, 16)}px;font-weight:${element.fontWeight || "400"};color:${element.textColor || "#111827"};text-align:${element.textAlign || "left"}">${escapeHtml(element.text || "")}</span></div>`;
    if (element.type === "image" && element.imageUri)
        return `<div ${attrs}><img src="${escapeHtml(element.imageUri)}" alt="${escapeHtml(element.accessibilityLabel || element.name || "Image")}" style="width:100%;height:100%;object-fit:${element.imageFit || "cover"}"/></div>`;
    if (element.type === "svg" && element.svgMarkup)
        return `<div ${attrs}>${element.svgMarkup}</div>`;
    return `<div ${attrs}></div>`;
}
function renderPage(project, page, index) {
...
export function createDigitalWebsiteExportState(now = Date.now()) { return { version: "22.6", initializedAt: now, updatedAt: now, revision: 1, exportDirectory: "dist/web", includeSourceMap: false, generateZip: true }; }
export function normalizeDigitalWebsiteExport(project) {
    const base = normalizeDigitalPublishingEditorIntegration(project);
    const now = Date.now();
    const fallback = createDigitalWebsiteExportState(now);
    const current = base.digitalWebsiteExport;
...
export function createDigitalWebsiteFiles(project) {
    const normalized = normalizeDigitalWebsiteExport(project);
    const deploy = normalized.digitalPublishingDeployment;
    const baseUrl = deploy.deployments.find(d => d.id === deploy.defaultDeploymentId)?.baseUrl.replace(/\/$/, "") || "https://example.com";
    const files = normalized.pages.map((page, index) => ({ path: fileFor(page, index), mimeType: "text/html", content: renderPage(normalized, page, index) }));
    ...
    if (deploy.generateSitemap)
        files.push({ path: "sitemap.xml", mimeType: "application/xml", content: `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${normalized.pages.map((p, i) => `<url><loc>${baseUrl}${routeFor(p, i)}</loc></url>`).join("")}</urlset>` });
    if (deploy.generateRobotsTxt)
        files.push({ path: "robots.txt", mimeType: "text/plain", content: `User-agent: *\nAllow: /\nSitemap: ${baseUrl}/sitemap.xml\n` });
    if (deploy.pwa.enabled)
        files.push({ path: "manifest.webmanifest", mimeType: "application/manifest+json", content: JSON.stringify({ name: deploy.pwa.appName, short_name: deploy.pwa.shortName, start_url: "/", display: deploy.pwa.display, theme_color: deploy.pwa.themeColor, background_color: deploy.pwa.backgroundColor }, null, 2) });
    else
        files.push({ path: "manifest.webmanifest", mimeType: "application/manifest+json", content: JSON.stringify({ name: normalized.name, short_name: normalized.name.slice(0, 12), start_url: "/", display: "browser" }, null, 2) });
    if (deploy.generateSecurityHeaders)
        files.push({ path: "_headers", mimeType: "text/plain", content: "/*\n  X-Content-Type-Options: nosniff\n  X-Frame-Options: SAMEORIGIN\n  Referrer-Policy: strict-origin-when-cross-origin\n  Permissions-Policy: camera=(), microphone=(), geolocation=()\n" });
...
export function certifyDigitalWebsiteExport(project, files = createDigitalWebsiteFiles(project)) {
    const normalized = normalizeDigitalWebsiteExport(project);
    const deploymentIssues = validateDigitalPublishingDeployment(normalized);
    const paths = new Set(files.map(f => f.path));
    const routes = normalized.pages.map(routeFor);
    ...
    const errors = checks.filter(c => !c.passed).length + deploymentIssues.filter(i => i.severity === "error").length;
    const warnings = deploymentIssues.filter(i => i.severity === "warning").length;
    const base = { version: "22.6", passed: errors === 0, generatedAt: Date.now(), errors, warnings, files: files.map(f => f.path), routes, checks };
    return { ...base, checksum: hash(JSON.stringify(base), "cert") };
}
export function createDigitalWebsiteExportPackage(project) { const files = createDigitalWebsiteFiles(project); const certification = certifyDigitalWebsiteExport(project, files); const base = { version: "22.6", projectId: project.id, projectName: project.name, files, certification }; return { ...base, checksum: hash(JSON.stringify(base)) }; }
export async function buildDigitalWebsiteZip(project) { const pkg = createDigitalWebsiteExportPackage(project); const zip = new JSZip(); pkg.files.forEach(file => zip.file(file.path, file.content)); zip.file("production-certification.json", JSON.stringify(pkg.certification, null, 2)); zip.file("phase22.6-export-manifest.json", JSON.stringify({ version: pkg.version, projectId: pkg.projectId, projectName: pkg.projectName, checksum: pkg.checksum, files: pkg.files.map(f => f.path) }, null, 2)); return zip.generateAsync({ type: "uint8array", compression: "DEFLATE", compressionOptions: { level: 9 } }); }
