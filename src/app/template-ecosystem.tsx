    ...
    <ScrollView contentContainerStyle={s.body}>
      ...
      {viewMode==="categories"?<View style={s.grid}>
        {filtered.map(category=><Pressable key={category.id} onPress={()=>setSelected(category)} style={[s.categoryCard,{width:columns===1?"100%":`${(100-(columns-1)*1.05)/columns}%` as any}]}>
          ...
          <View style={s.thumbnailStrip}>{category.templates.map((template,index)=><TemplateThumbnail key={template.metadata.id} template={template} index={index}/>)}</View>
      ...
      </View>:<View style={s.allGrid}>{filteredAll.map(({template,category},index)=><Pressable key={template.metadata.id} onPress={()=>open(template)} style={s.allTemplateCard}>
