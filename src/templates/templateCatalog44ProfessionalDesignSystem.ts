...
export function auditPhase44Template(template: ProfessionalTemplate): Phase44QualityReport {
  ...
  const checks = [
    ...
    { id: "palette", label: "Professional palette has at least four colors", passed: (template.metadata.palette?.length ?? 0) >= 4 },
    { id: "fonts", label: "Professional font pairing is declared", passed: (template.metadata.fonts?.length ?? 0) >= 2 },
...
export function phase44NormalizeTemplate(template: ProfessionalTemplate): ProfessionalTemplate {
  return {
    ...
    metadata: {
      ...template.metadata,
      version: "44.0.0",
      tags: Array.from(new Set([...template.metadata.tags, "", "professional design system", "quality audited"])),
