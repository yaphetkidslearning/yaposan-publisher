...
const makeTemplate = (spec: Spec, index: number): ProfessionalTemplate => {
  ...
  return {
    metadata: {
      ...
      description: `${spec.name} is a handcrafted design with composition ${mode + 1}, variant ${variant + 1}, editable typography, shapes, and production-ready spacing.`,
      tags: [spec.category, spec.subcategory, `composition-${mode + 1}`, `variant-${variant + 1}`, "", "editable", "professional"],
