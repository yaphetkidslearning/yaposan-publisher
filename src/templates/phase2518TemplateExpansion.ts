...
export type Phase2518CategoryGroup = {
...
export const PHASE2518_CATEGORY_GROUPS: readonly Phase2518CategoryGroup[] = [
...
export const PHASE2518_COLLECTION_TARGETS = [
...
type SubcategorySpec = { group: Phase2518CategoryGroup["id"]; subcategory: string; category: ProfessionalTemplateCategory; format: Format; industry: string };
...
const PALETTES = [
  ...
  { name:"Royal Violet", bg:"#24104F", surface:"#FCFAFF", accent:"#8B5CF6", ink:"#24104F", soft:"#EEE7FF" },
  { name:"Coral Editorial", bg:"#3A1720", surface:"#FFF9F7", accent:"#FF6B5E", ink:"#32151B", soft:"#FFE4DF" },
  { name:"Ocean Teal", bg:"#073642", surface:"#F5FEFF", accent:"#14B8A6", ink:"#083344", soft:"#D9FAF5" },
  { name:"Cobalt Modern", bg:"#0A2463", surface:"#FAFCFF", accent:"#2563EB", ink:"#102A56", soft:"#E1EBFF" },
  { name:"Rose Boutique", bg:"#4A1830", surface:"#FFF9FC", accent:"#E84A8A", ink:"#3B1628", soft:"#FCE2EE" },
  { name:"Copper Craft", bg:"#352116", surface:"#FFF9F2", accent:"#C56A2D", ink:"#321F14", soft:"#F6E2D2" },
  { name:"Lime Tech", bg:"#132A20", surface:"#FBFFF8", accent:"#84CC16", ink:"#1A2E22", soft:"#ECFAD7" },
  { name:"Slate Minimal", bg:"#1E293B", surface:"#F8FAFC", accent:"#64748B", ink:"#0F172A", soft:"#E2E8F0" },
  { name:"Sunset Orange", bg:"#431407", surface:"#FFF8F2", accent:"#F97316", ink:"#3B160A", soft:"#FFEDD5" },
  { name:"Aqua Creative", bg:"#083344", surface:"#F2FFFF", accent:"#06B6D4", ink:"#083344", soft:"#CFFAFE" },
] as const;
...
function createTemplate(spec:SubcategorySpec, variant:number):ProfessionalTemplate {
  const slug=safe(spec.subcategory); const id=`phase2518-${slug}-${variant+1}`; const palette=PALETTES[variant];
  const pageCount=spec.format.pages??1;
  return {
    metadata:{ id, name:`${palette.name} ${spec.subcategory.replace(/s$/,"")}`, category:spec.category, subcategory:spec.subcategory, industry:spec.industry, description:`A professional ${spec.subcategory.toLowerCase()} design with fully editable text, shapes, colors, tables, and layout. No person photography is included.`, tags:[spec.group,slug,"phase 25.18","editable","professional","no people",palette.name.toLowerCase()], pageSize:spec.format.pageSize, orientation:spec.format.orientation, previewColor:palette.accent, author:"Yaposan", version:"25.18", editable:true, featured:variant===0, trending:variant===2, access:variant===2?"premium":"free", createdAt:NOW, updatedAt:NOW, style:variant===0?"corporate":variant===1?"luxury":"creative", palette:[palette.bg,palette.surface,palette.accent,palette.ink], fonts:["Arial"], qualityScore:94+variant },
    pages:Array.from({length:pageCount},(_,pageIndex)=>createPage(spec,variant,pageIndex,`${id}-page-${pageIndex+1}`)),
  };
}

export const PHASE2518_TEMPLATES:ProfessionalTemplate[] = SPECS.flatMap(spec=>PALETTES.map((_,variant)=>createTemplate(spec,variant)));
export const PHASE2518_TEMPLATE_COUNT = PHASE2518_TEMPLATES.length;
export const PHASE2518_SUBCATEGORY_COUNT = SPECS.length;
export const PHASE2518_COUNTS_BY_SUBCATEGORY = Object.fromEntries(SPECS.map(spec=>[spec.subcategory,PHASE2518_TEMPLATES.filter(template=>template.metadata.subcategory===spec.subcategory).length]));
