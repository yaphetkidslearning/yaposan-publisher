...
function createRedesignedPage(template: ProfessionalTemplate, page: PublisherPage, pageIndex: number): PublisherPage {
  ...
  if (style === 0) {
    ...
    elements.push(text(`${id}-kicker`, template.metadata.subcategory?.toUpperCase() ?? template.metadata.category.toUpperCase(), tx, ty, tw, 28, 14, soft, "700", bodyFont, { letterSpacing: 2.4 }));
    ...
    elements.push(text(`${id}-kicker`, "CURATED COLLECTION", tx, H * 0.16, tw, 26, 13, primary, "800", bodyFont, { letterSpacing: 2.1 }));
    ...
    elements.push(text(`${id}-footer`, template.metadata.industry?.toUpperCase() ?? "YAPOSAN STUDIO", tx, H * 0.88, tw, 24, 12, primary, "800", bodyFont, { letterSpacing: 1.8 }));
  } else if (style === 2) {
    ...
    elements.push(text(`${id}-photo-label`, "VISUAL STORY", W * 0.59, H * 0.40, W * 0.25, 40, 18, paper, "900", headingFont, { textAlign: "center", letterSpacing: 2 }));
    elements.push(text(`${id}-kicker`, "DESIGNED TO STAND OUT", margin, H * 0.14, W * 0.43, 26, 13, primary, "800", bodyFont, { letterSpacing: 2 }));
    ...
    elements.push(text(`${id}-kicker`, template.metadata.category.toUpperCase(), margin * 1.5, H * 0.10, W - margin * 3, 26, 13, primary, "800", bodyFont, { textAlign: "center", letterSpacing: 3 }));
    ...
    elements.push(text(`${id}-footer`, "YAPOSAN / PROFESSIONAL TEMPLATE", margin * 1.5, H * 0.87, W - margin * 3, 26, 13, primary, "800", bodyFont, { textAlign: "center", letterSpacing: 2 }));
  } else if (style === 4) {
    ...
    elements.push(text(`${id}-kicker`, "MODERN • EDITABLE • READY", margin*1.5, H*0.20, W-margin*3, 28, 14, soft, "700", bodyFont, { letterSpacing: 2.2 }));
    ...
    elements.push(text(`${id}-kicker`, "FEATURED COLLECTION", margin, H*.08, W-margin*2, 26, 13, primary, "800", bodyFont, { letterSpacing: 2.2 }));
    ...
    elements.push(text(`${id}-footer`, "PROFESSIONAL TEMPLATE • FULLY EDITABLE", margin, H*.86, W-margin*2, 26, 13, primary, "800", bodyFont, { letterSpacing: 1.6 }));
  } else if (style === 6) {
    elements.push(rect(`${id}-band`, 0, H*.08, W, H*.20, primary));
    elements.push(text(`${id}-series`, "THE NEW PROFESSIONAL SERIES", margin, H*.13, W-margin*2, 28, 14, paper, "800", bodyFont, { letterSpacing: 2.3 }));
    ...
    elements.push(text(`${id}-brand`, "YAPOSAN DESIGN COLLECTION", margin, H*.05, W-margin*2, 28, 14, paper, "800", bodyFont, { letterSpacing: 2.4 }));
...
export function redesignTemplateForPhase25421(template: ProfessionalTemplate): ProfessionalTemplate {
  ...
  return {
    metadata: {
      ...
      tags: [...new Set([...template.metadata.tags, "premium redesign", "modern editorial", ""])],
    },
