...
function makeTemplate(spec: Spec, index: number): ProfessionalTemplate {
  ...
  const page: PublisherPage = { id: `${id}-page-1`, name: "Page 1", width: spec.width, height: spec.height, orientation: spec.orientation === "square" ? "portrait" : spec.orientation, sizeKey: spec.pageSize.toLowerCase().replace(/\s+/g, "-"), backgroundColor: palettes[index % palettes.length][1], margin: 24, bleed: DEFAULT_BLEED, elements: elements(spec, id, index) };
