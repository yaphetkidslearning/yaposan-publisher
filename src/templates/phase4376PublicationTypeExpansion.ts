...
function layout(prefix:string,c:Cat,n:number,pageNo:number,p:[string,string,string,string],fonts:[string,string],photo:string):PublisherElement[]{
 ...
 es.push(text(`${prefix}-kicker`,`${c.subcategory.toUpperCase()} · COLLECTION III`,left,titleY-H*.075,avail,H*.05,Math.max(8,H*.017),accent,"900",{fontFamily:fonts[1],letterSpacing:1.8}));
 es.push(text(`${prefix}-title`,`${NAMES[n-41]} ${c.subcategory.replace(/s$/,'')}`.toUpperCase(),left,titleY,avail,H*.15,Math.max(22,Math.min(56,H*.06)),ink,"900",{fontFamily:fonts[0],lineHeight:Math.max(26,H*.067)}));
...
function build(c:Cat,n:number):ProfessionalTemplate{const i=n-61,p=PALETTES[i%PALETTES.length],fonts=FONTS[i%FONTS.length],photo=PHOTOS[(i+c.slug.length)%PHOTOS.length];const pageCount=c.kind==="brochure"||c.kind==="newsletter"?2:c.kind==="social media kit"?3:1;const id=`phase4376-${c.slug}-${String(n).padStart(2,"0")}`;const pages:PublisherPage[]=Array.from({length:pageCount},(_,pageNo)=>{const pid=`${id}-p${pageNo+1}`;return{id:pid,name:pageCount===1?c.subcategory:`${c.subcategory} ${pageNo+1}`,width:c.width,height:c.height,orientation:c.orientation,sizeKey:c.pageSize.toLowerCase().replace(/[^a-z0-9]+/g,"-"),backgroundColor:p[0],margin:Math.max(18,Math.round(Math.min(c.width,c.height)*.04)),bleed:DEFAULT_BLEED,elements:[...layout(pid,c,n,pageNo,p,fonts,photo),...extras(pid,c,p,fonts)]};});return{metadata:{id,name:`${NAMES[i%NAMES.length]} ${c.subcategory.replace(/s$/,'')} Collection IV ${String(i+1).padStart(2,"0")}`,category:c.category,subcategory:c.subcategory,industry:c.industry,description:`A new Phase 24.3D-style ${c.kind} with a unique geometric system, editable typography, curated color, and publication-ready structure.`,tags:[c.kind,c.subcategory.toLowerCase(),"phase 24.3D","collection IV","fully editable","print ready",c.industry.toLowerCase()],pageSize:c.pageSize,orientation:c.orientation,previewColor:p[1],palette:[...p],fonts:[...fonts],author:"Yaposan Design Studio",version:"43.7.6",editable:true,featured:i%5===0,trending:i%7===0,access:i%4===0?"premium":"free",createdAt:NOW,updatedAt:NOW,style:(i%6===0?"corporate":i%6===1?"luxury":i%6===2?"minimal":i%6===3?"creative":i%6===4?"editorial":"bold"),qualityScore:100,masterTemplateId:id},pages};}
export const PHASE4376_PUBLICATION_TYPE_TEMPLATES:ProfessionalTemplate[]=CATS.flatMap(c=>Array.from({length:30},(_,i)=>build(c,i+61)));
export const PHASE4376_PUBLICATION_TYPE_COUNT=PHASE4376_PUBLICATION_TYPE_TEMPLATES.length;
export const PHASE4376_PUBLICATION_TYPES=CATS.map(c=>c.subcategory);
