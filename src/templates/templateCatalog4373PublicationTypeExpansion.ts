...
function build(c:Cat,n:number):ProfessionalTemplate{const i=n-21,p=PALETTES[i],fonts=FONTS[i%FONTS.length],photo=PHOTOS[(i+c.slug.length)%PHOTOS.length];const pageCount=c.kind==="brochure"||c.kind==="newsletter"?2:c.kind==="social media kit"?3:1;const id=`phase4373-${c.slug}-${String(n).padStart(2,"0")}`;const pages:PublisherPage[]=Array.from({length:pageCount},(_,pageNo)=>{const pid=`${id}-p${pageNo+1}`;return{id:pid,name:pageCount===1?c.subcategory:`${c.subcategory} ${pageNo+1}`,width:c.width,height:c.height,orientation:c.orientation==="square"?"portrait":c.orientation,sizeKey:"custom",backgroundColor:p[0],margin:Math.max(18,Math.round(Math.min(c.width,c.height)*.04)),bleed:DEFAULT_BLEED,elements:[...layout(pid,c,n,pageNo,p,fonts,photo),...extras(pid,c,p,fonts)]};});return{metadata:{id,name:`${NAMES[i]} ${c.subcategory.replace(/s$/,'')} Collection II`,category:c.category,subcategory:c.subcategory,industry:c.industry,description:`A new style ${c.kind} with a unique geometric system, editable typography, curated color, and publication-ready structure.`,tags:[c.kind,c.subcategory.toLowerCase(),"","collection II","fully editable","print ready",c.industry.toLowerCase()],pageSize:c.pageSize,orientation:c.orientation,previewColor:p[1],palette:[...p],fonts:[...fonts],author:"Yaposan Design Studio",version:"43.7.3",editable:true,featured:i%5===0,trending:i%7===0,access:i%4===0?"premium":"free",createdAt:NOW,updatedAt:NOW,style:(i%6===0?"corporate":i%6===1?"luxury":i%6===2?"minimal":i%6===3?"creative":i%6===4?"editorial":"bold"),qualityScore:100,masterTemplateId:id},pages};}
