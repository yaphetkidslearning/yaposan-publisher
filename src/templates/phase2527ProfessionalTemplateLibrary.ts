...
const makeTemplate = (spec: Spec, index: number): ProfessionalTemplate => {
  ...
  return {
    metadata: {
      ...
      description: `${spec.name} is a Phase 25.27 handcrafted template with a distinct layout, editable text, vector shapes, and professional production spacing.`,
      tags: [spec.category, spec.subcategory, `family-${family + 1}`, `layout-${variant + 1}`, "phase 25.27", "editable", "professional"],
