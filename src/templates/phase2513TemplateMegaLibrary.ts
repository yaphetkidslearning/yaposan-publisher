...
export type Phase2513CategoryGroup = {
...
export const PHASE2513_CATEGORY_GROUPS: readonly Phase2513CategoryGroup[] = [
...
export const PHASE2513_COLLECTION_TARGETS = [
...
type SubcategorySpec = { group: Phase2513CategoryGroup["id"]; subcategory: string; category: ProfessionalTemplateCategory; format: Format; industry: string };
...
const PALETTES = [
  { name:"Midnight Blue", bg:"#071B33", surface:"#FFFFFF", accent:"#27B5E8", ink:"#0F172A", soft:"#E8F6FC" },
  { name:"Executive Gold", bg:"#171717", surface:"#FFFDF7", accent:"#C89B3C", ink:"#171717", soft:"#F7EFD9" },
  { name:"Emerald Studio", bg:"#073B3A", surface:"#F7FFFC", accent:"#20C997", ink:"#12302D", soft:"#DDF8EE" },
] as const;
...
function createPage(spec:SubcategorySpec, variant:number, pageIndex:number, id:string):PublisherPage {
  ...
  const elements:PublisherElement[] = [
    el(`${id}-background`,"Background","rectangle",0,0,W,H,1,{fillColor:variant===1?p.surface:p.bg,borderWidth:0,locked:true}),
    el(`${id}-accent-bar`,"Accent Bar","rectangle",0,0,variant===2?Math.max(18,W*.025):W,variant===2?H:headerH,2,{fillColor:p.accent,borderWidth:0}),
    ...
    el(`${id}-brand-name`,"Business Name","text",margin+Math.max(38,Math.min(W,H)*.085),margin,W-margin*2,42,4,{text:"YOUR COMPANY",fontFamily:"Arial",fontSize:Math.max(11,titleSize*.34),fontWeight:"900",letterSpacing:1.4,textColor:variant===1?p.ink:p.surface}),
    el(`${id}-title`,"Headline","text",margin,H*.23,W-margin*2,H*.2,5,{text:copy.title,fontFamily:"Arial",fontSize:titleSize,fontWeight:"900",letterSpacing:variant===1?.2:1.1,textColor:variant===1?p.ink:p.surface,textAlign:spec.subcategory==="Certificates"?"center":"left",verticalJustification:"center"}),
    ...
    el(`${id}-subtitle`,"Editable Content","text",margin,H*.5,W-margin*2,H*.25,7,{text:copy.subtitle,fontFamily:"Arial",fontSize:Math.max(10,Math.min(25,W/42)),fontWeight:"500",lineHeight:1.35,textColor:variant===1?p.ink:p.surface,textAlign:spec.subcategory==="Certificates"?"center":"left"}),
    ...
    el(`${id}-contact`,"Contact Details","text",W*.52,H-footerH-margin+footerH*.22,W*.43-margin,footerH*.55,10,{text:"hello@example.com  ·  www.example.com",fontFamily:"Arial",fontSize:Math.max(8,Math.min(14,W/80)),fontWeight:"700",textColor:variant===1?p.ink:p.surface,textAlign:"right"}),
  ];
  ...
  return { id, name: spec.format.pages && spec.format.pages>1 ? `Page ${pageIndex+1}` : spec.subcategory, width:W, height:H, orientation:spec.format.orientation==="landscape"?"landscape":"portrait", sizeKey:"custom", backgroundColor:variant===1?p.surface:p.bg, margin, bleed:DEFAULT_BLEED, elements };
}

function createTemplate(spec:SubcategorySpec, variant:number):ProfessionalTemplate {
  const slug=safe(spec.subcategory); const id=`phase2513-${slug}-${variant+1}`; const palette=PALETTES[variant];
  const pageCount=spec.format.pages??1;
  return {
    metadata:{ id, name:`${palette.name} ${spec.subcategory.replace(/s$/,"")}`, category:spec.category, subcategory:spec.subcategory, industry:spec.industry, description:`A professional ${spec.subcategory.toLowerCase()} design with fully editable text, shapes, colors, tables, and layout. No person photography is included.`, tags:[spec.group,slug,"phase 25.13","editable","professional","no people",palette.name.toLowerCase()], pageSize:spec.format.pageSize, orientation:spec.format.orientation, previewColor:palette.accent, author:"Yaposan", version:"25.13", editable:true, featured:variant===0, trending:variant===2, access:variant===2?"premium":"free", createdAt:NOW, updatedAt:NOW, style:variant===0?"corporate":variant===1?"luxury":"creative", palette:[palette.bg,palette.surface,palette.accent,palette.ink], fonts:["Arial"], qualityScore:94+variant },
    pages:Array.from({length:pageCount},(_,pageIndex)=>createPage(spec,variant,pageIndex,`${id}-page-${pageIndex+1}`)),
  };
}

export const PHASE2513_TEMPLATES:ProfessionalTemplate[] = SPECS.flatMap(spec=>PALETTES.map((_,variant)=>createTemplate(spec,variant)));
export const PHASE2513_TEMPLATE_COUNT = PHASE2513_TEMPLATES.length;
export const PHASE2513_SUBCATEGORY_COUNT = SPECS.length;
export const PHASE2513_COUNTS_BY_SUBCATEGORY = Object.fromEntries(SPECS.map(spec=>[spec.subcategory,PHASE2513_TEMPLATES.filter(template=>template.metadata.subcategory===spec.subcategory).length]));
