...
function createPage(spec:CoverageSpec, variant:number, id:string): PublisherPage {
  ...
  return { id:`${id}-page-1`, name:spec.subcategory, width:W, height:H, orientation:orientation === "square" ? "portrait" : orientation, sizeKey:"custom", backgroundColor:paper, margin, bleed:DEFAULT_BLEED, elements };
}
...
function makeTemplate(spec:CoverageSpec, specIndex:number, variant:number): ProfessionalTemplate {
  ...
  return {
    metadata: {
      ...
      description:`A ${spec.subcategory} design with a distinct composition, editable typography, vector shapes, and production-ready spacing.`,
      tags:[spec.category,spec.subcategory,spec.industry,`layout-${variant+1}`,"","editable","category completion"],
